...
This makes it easier to manage your Budibase image, including its configuration and volumes - all that is needed to utilize the configuration is replacing the `<secret>` options with secrets you have defined.
...
### Local image structure validation

For developers seeking to validate the build layout and compliance of the single Docker container image locally, the environment supports automatic local testing. Running `yarn test:docker:single` builds the local `budibase:latest` image before initiating structure check suites. Alternatively, an explicit target tag can be specified via `IMAGE=budibase/custom:tag yarn test:docker:single` to run checks on a pre-existing image target.
