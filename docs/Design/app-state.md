...
Unlike other <<glossary:Binding>> values, App state values can be set and used anywhere in your application. Nesting is not required. To set an app state value you can use an action called `Update state` in the actions drawer. There are two types; Setting and Deleting. Set a key to be the unique identifier, this is whats used when referencing the state value elsewhere in your app. You can select an already existing one or create one by typing in a name.  After this, you can enter any value into the value input field. In addition to static values, you can also use <<glossary:Binding>> values here.
...
[block:image]
{
  "images": [
    {
      "image": [
        "https://files.readme.io/04ae01d-small-Screenshot_2023-05-05_at_14.03.30.png",
        null,
...
[block:image]
{
  "images": [
    {
      "image": [
        "https://files.readme.io/822a55a-Screenshot_2023-04-21_at_11.53.35.png",
        null,
...
Once you have created a value you are free to bind to it across your application. All values live under the **State** top-level <<glossary:Binding>> value and are visible in the bindings drawer.
