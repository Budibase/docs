...
Click on the user you want to change, and you'll be presented with a screen not much different than the [Adding Users](doc:adding-users) screen. When you scroll down a little bit you can configure `Development` and `Administration` roles. Keep in mind the `Administration` role requires the `Development` role also to be enabled.
