...
In the above example, the "encodeURI" function is called with the string "<https://myurl?Hello> There". It encodes the string by replacing the space character with `%20`, which is the escape sequence for a space in URI encoding. 
...
For example, if you use the "escape" function with the argument "<https://myurl?Hello+There">, it will return "<https://myurl?Hello%20There">. In this case, the space character ' ' is replaced with '%20', which is the escape sequence for a space in URLs.
...
For example, if you use the "decodeURI" function with the argument "<https://myurl?Hello%20There">, it will return "<https://myurl?Hello> There". In this case, the `%20` escape sequence is replaced with a space character.
...
For example, if you use the "urlResolve" function with the arguments "<https://myurl>" and "/api/test", it will return "<https://myurl/api/test">. The function combines the base URL and the href URL to form a complete, resolved URL.
...
This example demonstrates how to use the "urlParse" function by passing the URL string '<https://myurl/api/test'>. The function will parse the URL and return an object representing the parsed components of the URL.
...
This example removes the query string from the URL '<https://myurl/api/test?foo=bar>' and returns '<https://myurl/api/test'>.

## Strip protocol

The "stripProtocol" function removes the protocol (e.g., "http\://" or "<https://">) from a given URL. It takes a URL as input and returns the URL without the protocol. This function is useful when you want to display a URL without the protocol, especially in cases where you want to display media on secure connections.
...
This example removes the protocol from the URL '<https://myurl/api/test>' and returns 'myurl/api/test'.