...
Clicking on this icon will open the environment variable menu, which allows you to select from a list of variables that have been created. You will also have the option to [create a new environment variable](#creating-an-environment-variable), in exactly the same way as you can in the portal, this is provided as a convenience feature incase there are any variables that you may have forgotten to create.
...
Another useful feature of environment variables is their ability to be used in [queries](doc:custom-queries) as well as datasource configuration. If you open the [binding drawer](doc:bindings) you will see a new environment section has been added, as seen below.
...
These can also be used in an [SQL query](doc:custom-queries), in the same format as the binding drawer, like `{{ env.env_variable }}` this is shown below.
