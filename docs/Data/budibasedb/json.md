...
Then you can use a repeater on _Column.array1_, then put a repeater block inside that and target _array1.array2_, then put another repeater block inside that and target _array2.array3_, and finally get out the values. You can chain this as long as you like and the schema will always be available.
...
If you have a form targetting a subsection of your JSON field, then the best way to save your full row value is to write a small JS binding and make use of the new _Form.Value_ binding (which is an object of the whole value of the form) to insert the value into your JSON field. This is only really needed when you are iterating over JSON arrays and want to update one of the array elements.
